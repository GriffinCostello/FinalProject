...
function createAnime(results) {
	...
	elemButton.onclick = function() {fetchCharacters(showId , elemDiv);};
...
function fetchCharacters(showId, elemDiv) {
  console.log()
 
  fetch('https://api.jikan.moe/v3/anime/' + showId + '/characters_staff')
    .then(response => response.json())
    .then(data => showCharacters(data, elemDiv));
    
  
} // fetch episodes

function showCharacters (data, elemDiv) {
    console.log(data);
